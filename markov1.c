...
void build(char *prefix[NPREF], FILE *f)//проходимось по файлу та створюємо елементи
{
    char buf[100], fmt[10];
...
void generate(int nwords)// генерує текст закінчує програму або коли доходить до кінця або коли доходить до ліміту слів
{
    char NONWORD[] = "\n";
    State *sp;
    Suffix *suf;
...
int main(void)// будуємо статистику, додаємо індикатор кінця, генеруємо текст
{
